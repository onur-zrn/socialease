...
class Posts extends Component {
  ...
  getOwnPosts = async () => {
    try {
      const response = await getOwnPosts(this.props.userDetails.userId);
      ...
      this.setState({ posts: deneme, currentSection: "posts" });
    } catch (e) {
      console.log(e);
    }
  };
  ...
  getLikedPosts = async () => {
    try {
      ...
      let deneme = response.data;
      if (deneme === "" || deneme === null || deneme === undefined) {
        this.setState({ posts: [], currentSection: "likes" });
      } else {
        deneme = deneme.map((d) => {
          const d2 = {
            ...d,
        ...
        console.log(deneme);
        this.setState({ posts: deneme, currentSection: "likes" });
      }
    } catch (e) {
      console.log(e);
    }
  ...
  render() {
    ...
    let postButton = (
      <div style={grayButton} onClick={() => this.getOwnPosts()}>
        Posts
      </div>
    );
    let likeButton = (
      <div style={grayButton} onClick={() => this.getLikedPosts()}>
        Likes
      </div>
    );
    if (this.state.currentSection === "posts") {
      postButton = (
        <div style={redButton} onClick={() => this.getOwnPosts()}>
          Posts
        </div>
      );
      likeButton = (
    ...
    } else if (this.state.currentSection === "likes") {
      postButton = (
        <div style={grayButton} onClick={() => this.getOwnPosts()}>
          Posts
        </div>
      ...
      likeButton = (
        <div style={redButton} onClick={() => this.getLikedPosts()}>
          Likes
        </div>
      );
    }
